...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Exoplanet_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Exoplanet_Breadcrumb_Trail.
 * @return void
 */
function exoplanet_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'exoplanet_breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new Exoplanet_Breadcrumb_Trail( $args );

	return $breadcrumb->exoplanet_trail();
}
 ...
 * @access public
 */
class Exoplanet_Breadcrumb_Trail {
	...
	public function __toString() {
		return $this->exoplanet_trail();
	}

	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Exoplanet_Breadcrumb_Trail::exoplanet_add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Exoplanet_Breadcrumb_Trail::exoplanet_set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Exoplanet_Breadcrumb_Trail::exoplanet_set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->exoplanet_set_labels();
		$this->exoplanet_set_post_taxonomy();

		// Let's find some items to add to the trail!
		$this->exoplanet_add_items();
	}
	 ...
	 * @return string
	 */
	public function exoplanet_trail() {
	 ...
	 * @return void
	 */
	protected function exoplanet_set_labels() {

		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'exoplanet' ),
			'home'                => esc_html__( 'Home',                                  'exoplanet' ),
			'error_404'           => esc_html__( '404 Not Found',                         'exoplanet' ),
			'archives'            => esc_html__( 'Archives',                              'exoplanet' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'exoplanet' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'exoplanet' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'exoplanet' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'exoplanet' ),
	 ...
	 * @return void
	 */
	protected function exoplanet_set_post_taxonomy() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_items() {

		// If viewing the front page.
		if ( is_front_page() ) {
			$this->exoplanet_add_front_page_items();
		}
		...
		// If not viewing the front page.
			...
			$this->exoplanet_add_network_home_link();
			$this->exoplanet_add_site_home_link();

			// If viewing the home/blog page.
			if ( is_home() ) {
				$this->exoplanet_add_posts_page_items();
			}

			// If viewing a single post.
			elseif ( is_singular() ) {
				$this->exoplanet_add_singular_items();
			}
			...
			elseif ( is_archive() ) {
				...
				if ( is_post_type_archive() )
					$this->exoplanet_add_post_type_archive_items();

				elseif ( is_category() || is_tag() || is_tax() )
					$this->exoplanet_add_term_archive_items();

				elseif ( is_author() )
					$this->exoplanet_add_user_archive_items();

				elseif ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
					$this->exoplanet_add_minute_hour_archive_items();

				elseif ( get_query_var( 'minute' ) )
					$this->exoplanet_add_minute_archive_items();

				elseif ( get_query_var( 'hour' ) )
					$this->exoplanet_add_hour_archive_items();

				elseif ( is_day() )
					$this->exoplanet_add_day_archive_items();

				elseif ( get_query_var( 'w' ) )
					$this->exoplanet_add_week_archive_items();

				elseif ( is_month() )
					$this->exoplanet_add_month_archive_items();

				elseif ( is_year() )
					$this->exoplanet_add_year_archive_items();

				else
					$this->exoplanet_add_default_archive_items();
			}

			// If viewing a search results page.
			elseif ( is_search() ) {
				$this->exoplanet_add_search_items();
			}

			// If viewing the 404 page.
			elseif ( is_404() ) {
				$this->exoplanet_add_404_items();
			}
		}

		// Add paged items if they exist.
		$this->exoplanet_add_paged_items();
	 ...
	 * @return void
	 */
	protected function exoplanet_add_rewrite_front_items() {
		global $wp_rewrite;

		if ( $wp_rewrite->front )
			$this->exoplanet_add_path_parents( $wp_rewrite->front );
	}
	 ...
	 * @return void
	 */
	protected function exoplanet_add_paged_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_network_home_link() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_site_home_link() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_front_page_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_posts_page_items() {
		...
		if ( 0 < $post->post_parent )
			$this->exoplanet_add_post_parents( $post->post_parent );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_singular_items() {
		...
		// If the post doesn't have a parent, get its hierarchy based off the post type.
			...
			$this->exoplanet_add_post_hierarchy( $post_id );

		// Display terms for specific post type taxonomy if requested.
		if ( !empty( $this->post_taxonomy[ $post->post_type ] ) )
			$this->exoplanet_add_post_terms( $post_id, $this->post_taxonomy[ $post->post_type ] );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_term_archive_items() {
		global $wp_rewrite;
		...
		if ( false !== $taxonomy->rewrite ) {
			...
			if ( $taxonomy->rewrite['with_front'] && $wp_rewrite->front )
				$this->exoplanet_add_rewrite_front_items();

			// Get parent pages by path if they exist.
			$this->exoplanet_add_path_parents( $taxonomy->rewrite['slug'] );
			...
			if ( $taxonomy->rewrite['slug'] ) {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						$post_types = $this->exoplanet_get_post_types_by_slug( $match );
		...
		if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
			$this->exoplanet_add_term_parents( $term->parent, $term->taxonomy );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_post_type_archive_items() {
		...
		if ( false !== $post_type_object->rewrite ) {
			...
			if ( !empty( $post_type_object->rewrite['slug'] ) )
				$this->exoplanet_add_path_parents( $post_type_object->rewrite['slug'] );
		}
	 ...
	 * @return void
	 */
	protected function exoplanet_add_user_archive_items() {
		global $wp_rewrite;
		...
		if ( !empty( $wp_rewrite->author_base ) )
			$this->exoplanet_add_path_parents( $wp_rewrite->author_base );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'exoplanet' ) ) );
	}
	 ...
	 * @return void
	 */
	protected function exoplanet_add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'exoplanet' ) ) );
	}
	 ...
	 * @return void
	 */
	protected function exoplanet_add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'exoplanet' ) ) );
	}
	 ...
	 * @return void
	 */
	protected function exoplanet_add_day_archive_items() {

		// Add $wp_rewrite->front to the trail.
		$this->exoplanet_add_rewrite_front_items();

		// Get year, month, and day.
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'exoplanet' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'exoplanet' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'exoplanet' ) ) );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'exoplanet' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'exoplanet' ) ) );
	 ...
	 * @return void
	 */
	protected function exoplanet_add_month_archive_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_year_archive_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_default_archive_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_search_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_404_items() {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_post_parents( $post_id ) {
		$parents = array();
	 ...
	 * @return void
	 */
	protected function exoplanet_add_post_hierarchy( $post_id ) {
		...
		if ( 'post' === $post_type ) {
			...
			$this->exoplanet_map_rewrite_tags( $post_id, get_option( 'permalink_structure' ) );
		}
	 ...
	 * @return void
	 */
	protected function exoplanet_get_post_types_by_slug( $slug ) {
	 ...
	 * @return void
	 */
	protected function exoplanet_add_post_terms( $post_id, $taxonomy ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			if ( 0 < $term->parent )
				$this->exoplanet_add_term_parents( $term->parent, $taxonomy );
	 ...
	 * @return void
	 */
	function exoplanet_add_path_parents( $path ) {
		...
		if ( !empty( $post ) ) {
			$this->exoplanet_add_post_parents( $post->ID );
		}
	 ...
	 * @return void
	 */
	function exoplanet_add_term_parents( $term_id, $taxonomy ) {
	 ...
	 * @return array
	 */
	protected function exoplanet_map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'exoplanet' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'exoplanet' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'exoplanet' ) ) ) );
				...
				elseif ( '%category%' == $tag ) {
					...
					$this->exoplanet_add_post_terms( $post_id, 'category' );
				}
			}
		}
	}
